...
*** JUNKYARD Thoughs on doing the layout
    ...
    |               | Title        -1 |                |           |
    |               | Top Ruler    -2 |                |           |
    | Left Ruler -2 | Graph        -3 | Right Ruler -2 | Legend -1 |
    |               | Bottom Ruler -2 |                |           |
    |               | Caption      -1 |                |           |
    ...
    Some adjacent boxes are at a fixed distance from
    their neigbor, and some can be "springy", and so
    we denote this aspect in the linkages.
    ...
    Some boxes, like the Rulers, will take their with
    and hight based on the dominate they are connected to.
    Others, like the title and caption and legend boxes,
    are floating.

    And so we have the following box states:
    - floating - if not floating, will get either its width
      or height from the dominate neighbor.
    - spring - in relation to neighbor, spring or fixed
